...
void loop() {
  ...
  EVERY_N_MILLISECONDS( 10 ) {
    ...
    if(animationIsOn && lightbulbIsOn) {      
      runAnimation();
    }else{
      updateLightbulb();  
    }
...
void cha_switch_on_setter(const homekit_value_t v) {
    bool isOn                        = v.bool_value;
    cha_switch_on.value.bool_value   = isOn;

    animationIsOn       = v.bool_value;
...
void runAnimation() 
{
  ...
  // THIS IS JUST A RAINBOW ANIMATION
  static int gHue         = currentHue;
